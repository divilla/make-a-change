...
For Go validation, first read the owning module's Makefile, CI, and `AGENTS.md`.
Use its documented checks. Where equivalent commands are needed, these are Go
examples, run from that module directory (not a repository root without `go.mod`):
...
Use `gofmt` on the Go files you change and the project's configured Go linter.
Check integration-test prerequisites before choosing the full test command.
The JavaScript package-manager commands elsewhere in this template apply only
to JavaScript/TypeScript packages; do not run them to validate a Go-only change.
...
```go
// Key types to use or extend
// {relevant Go type definitions from codebase exploration}
```
